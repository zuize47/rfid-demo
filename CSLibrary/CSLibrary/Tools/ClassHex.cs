...
namespace CSLibrary.Tools
{
	...
	public class HexEncoding
	{
				...
				else
				{
					switch (leftUshortLength)
					{
						case 1:
							...
							break;
						case 2:
							hex = new String(new Char[] { newString[j], newString[j + 1], '0', '0' });
							break;
						case 3:
							hex = new String(new Char[] { newString[j], newString[j + 1], newString[j + 2], '0' });
							break;
						default: break;
					}
		...
		public static Int16 ToInt16(byte[] buffer, int offset)
		{
			...
			return (Int16)(buffer[offset] << 8 | buffer[offset + 1]);
...
/*
		...
		{
			return (UInt64)(buffer[offset] << 46 | buffer[offset + 1] << 38 | buffer[offset + 2] << 30 | buffer[offset + 3] | buffer[offset] << 24 | buffer[offset + 1] << 16 | buffer[offset + 2] << 8 | buffer[offset + 3]);
			/*
						if (buffer == null || buffer.Length == 0)
							return 0;
